...
解析健壮性（均由 samples/ 下三份真实课表与变体用例覆盖）：
  - 表头识别兼容 星期一 / 周一 / 周1 / 礼拜一 / 星期7；
  - 周次兼容 单双周（1-16单周 / 1-16双周 / 1-16(单) / 奇偶周），
    以及 "1-16周"、"第1-16周" 等带字写法；
  - 节次分隔兼容 "-"、","、"、"、"~"，并兼容全角括号与全角数字；
  - 教师职称括号兼容中文全角括号（唐心智（教授）），也容忍括号被换行
    打断（唐心智\n(教授)）；
  - 多人授课按 范钧(副教授),李鹏程(讲师) 解析为 "范钧,李鹏程"，
    不会把第二位教师并进课程名；
...
`samples/` 下三份真实课表（刘雨昂 37 门 / 冯海洋 57 门 / 王明阳 49 门）是
...
# 教师行，如 "唐心智(教授)"；兼容全角括号，职称括号可能被换行打断（合并后匹配）。
# 姓名按 2-8 字收：少数民族姓名（阿依古丽·买买提）会超过 4 字。
TEACHER_TITLES = (
    "教授", "副教授", "讲师", "助教", "未评级", "研究员", "副研究员",
    "高级工程师", "工程师", "实验师", "高级实验师", "外教", "讲师（外聘）",
)
_TEACHER_TITLE_ALT = "|".join(map(re.escape, sorted(TEACHER_TITLES, key=len, reverse=True)))
_TEACHER_NAME = r"[\u4e00-\u9fa5·]{2,8}"
# 单个教师："唐心智(教授)"；姓名与括号之间允许空白（职称括号被换行打断后
# 行会被合并成 "唐心智 (教授)"）
...
# 多人授课："范钧(副教授),李鹏程(讲师)" / "张三(讲师)、李四(教授)" /
# 空格分隔的 "张三(讲师) 李四(教授)"。
...
# 文件名常带后缀（学生个人课表_2502020411(1).xls），锚定后会导致学号为空；
...
def _extract_student_id(file_name: str) -> str:
    ...
    教务系统导出的文件名形态很多：`学生个人课表_2307724110.xls`、
    `学生个人课表_2502020411(1).xls`、`课表_2502020411（1）.xls`。
