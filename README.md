...
| 文件 | 覆盖内容 |
| --- | --- |
| `test_parser_robustness.py` | 课表解析：五份样例（37/57/49/39/40 门）元信息与课程数基线 + 备注行反查、跨节次去重、无地点课程、同课换教室、多人授课、职称括号换行、学号文件名后缀、单双周/表头写法/全角字符/空行异常/无空行多课程/合并单元格、格式嗅探与损坏文件报错 |
...
**改之前先读这句话**：`samples/desensitized/` 下的五份 `.xls` 来自真实教务系统导出
（37 门国际商务 / 57 门电信 / 49 门材科 / 39 门机器人 / 40 门物流管理），`test_parser_robustness.py` 里有它们的黄金基线
...
再改解析器让用例通过，最后跑 `pytest -q` 确认五份样例的基线没有漂移。
