...
- **空闲甘特图**：行=成员，列=星期×时段。颜色含义：
  ...
  - 可导出带填充色的 Excel 甘特图。
- **学期日历覆盖**：通过日期选择框登记法定假日 `off` 与周末补课 `class maps_to` 映射，支持一键生成互补调休项，也可从内置 2025/2026 或在线 `chinese-days` 数据导入国家法定节假日与调休上班日并预览确认。排班仍按逻辑周次运行，导出与甘特图显示真实日期；原放假日标红，补课日按所代表的逻辑工作日课表判定忙闲。
